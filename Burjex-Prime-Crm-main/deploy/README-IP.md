...
> **Windows Server all-in-one:** if you host Docker *and* the MT5 bridge on one
> Windows Server (Docker Desktop Linux containers), use the pack at
> [`deploy/windows-server/`](./windows-server/README.md). Those PowerShell
> scripts **always** pass `--env-file` — never recreate the stack without it
> (empty `BT_*` / `DATABASE_URL` crash market-data and stall `:4300` ingest).
...
box (or, on all-in-one Windows Server, posts to `http://127.0.0.1:4300`).
