...
class TraderNav {
  static void openTrade(BuildContext context, String symbol) {
    ...
    if (router != null) {
      router.go('/new-order?symbol=${Uri.encodeComponent(symbol)}');
      return;
    }
    ...
    Navigator.of(context).push(
      MaterialPageRoute(builder: (_) => NewOrderScreen(symbol: symbol)),
    );
  }
  ...
  static void openChart(BuildContext context, String symbol) {
    final router = GoRouter.maybeOf(context);
    if (router != null) {
      router.go('/chart?symbol=${Uri.encodeComponent(symbol)}');
      return;
    }
    final embed = BTraderEmbedNav.maybeOf(context);
    if (embed != null) {
